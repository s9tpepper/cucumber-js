var SummaryFormatter = function (options) {
  ...
  self.getScenariosSummary = function getScenariosSummary() {
    var scenariosSummary       = "";
    ...
    scenariosSummary += scenarioCount + " scenario" + (scenarioCount != 1 ? "s" : "");
    if (scenarioCount > 0 ) {
      ...
      scenariosSummary += " (" + details.join(', ') + ")";
    }
    return scenariosSummary;
  };

  self.logScenariosSummary = function logScenariosSummary() {
    self.log(self.getScenariosSummary());
    self.log("\n");
  };

  self.getStepsSummary = function getStepsSummary() {
    var stepsSummary       = "";
    ...
    stepsSummary += stepCount + " step" + (stepCount != 1 ? "s" : "");
    if (stepCount > 0) {
      ...
      stepsSummary += " (" + details.join(', ') + ")";
    }
    return stepsSummary;
  };

  self.logStepsSummary = function logStepsSummary() {
    self.log(self.getStepsSummary());
    self.log("\n");
  };
